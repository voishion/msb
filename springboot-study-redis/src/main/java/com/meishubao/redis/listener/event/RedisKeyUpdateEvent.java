...
/**
 * {@link RedisKeyUpdateEvent} is a Redis specific {@link ApplicationEvent} published when a particular key in Redis
 * expires. It can hold the value of the update key next to the key, but is not required to do so.
 *
 * @author Christoph Strobl
 * @author Mark Paluch
 * @since 1.7
 */
public class RedisKeyUpdateEvent<T> extends RedisKeyspaceEvent {
	...
	/**
	 * Creates new {@link RedisKeyUpdateEvent}.
	 *
	 * @param key the update key.
	 */
	public RedisKeyUpdateEvent(byte[] key) {
		this(key, null);
	}
	 ...
	 *
	 * @param key the update key.
	 * @param value the value of the update key. Can be {@literal null}.
	 */
	public RedisKeyUpdateEvent(byte[] key, @Nullable Object value) {
		this(null, key, value);
	}
	 ...
	 * @since 1.8
	 */
	public RedisKeyUpdateEvent(@Nullable String channel, byte[] key, @Nullable Object value) {
		super(channel, key);
