...
public class ProxyFetchFactory {
    ...
    private CacheService<String, HttpHost> cacheService = SpringContextHolder.getBean(CacheService.class);
    ...
    public void buildProxy() {
        ...
        for (AbstractProxyFetcher proxyService : proxyServices) {
            ...
            try {
                ...
                log.error("fetch " + proxyService.getBusiness() + " proxy meet error", e);
                continue;
            }
        ...
        log.info("valid host total size: " + getProxyHosts().size());
    }
    ...
    private boolean checkHost(final HttpProxy httpProxy) {
        ...
        log.info(httpProxy.getProvider() + " - " + httpProxy.getAddress() + " : " + isValid);
        if (isValid) {
            cacheService.put(httpProxy.getAddress() + ":" + httpProxy.getPort(), httpProxy.toHost());
            log.info("cache size:" + cacheService.size());
        }
        return isValid;
    }
